  ...
  template: `
    <div class="p-4">
      ...
      <mat-dialog-content>
        <form [formGroup]="form">
          ...
          <div class="mb-3">
            ...
            <input formControlName="email" type="text" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
            <div class="p-1">
              <span *ngIf="f.email?.invalid && (f.email?.dirty || f.email?.touched)">
                <span *ngIf="f.email?.errors?.['required']" class="text-danger fs-12">Email address is required</span>
              </span>
            </div>
          </div>
          <div class="mb-3">
      ...
      <mat-dialog-actions>
        ...
        <button class="text-sm px-4 py-2 bg-blue-600 border-0  rounded hover:bg-gray-300" (click)="addMember()">Add</button>
      </mat-dialog-actions>
    </div>
  `,
  styles: ``
